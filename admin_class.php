...
Class Action {
	...
	function save_project(){
		...
		$uploadFolder = 'assets/files/';
		//  echo $uploadFolder;
		//  exit;
		$dbfilename = time(). basename($_FILES['uploaded_files']['name']);
		$uploadedFile = $uploadFolder . $dbfilename;
		//print_r($dbfilename);
//exit;
		 $allowedExtensions = array('pdf', 'docx', 'zip', 'rar', 'txt', 'pptx', 'xlsx', 'txt', 'mp3', 'wav', 'mp4', 'avi', 'mkv', );
		$fileExtension = strtolower(pathinfo($uploadedFile, PATHINFO_EXTENSION));
		if (in_array($fileExtension, $allowedExtensions)) {
			if (move_uploaded_file($_FILES['uploaded_files']['tmp_name'], $uploadedFile)) {

				$data .= ", uploaded_files='$dbfilename' ";
				// File uploaded successfully
			//	echo "File upload Successfully";
			//	echo '<script>window.location.href = "index.php?page=project_list";</script>';
			} else {
				//$error = $_FILES['uploaded_files']['error'];
				//echo "Error: File upload failed with error code: $error";
			}

		} else {
			//echo "Error: Only PDF, DOC, ZIP, and RAR files are allowed.";
		}
